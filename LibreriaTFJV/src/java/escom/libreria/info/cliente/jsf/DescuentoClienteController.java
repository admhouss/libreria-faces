...
import escom.libreria.info.cliente.ejb.DescuentoClienteFacade;
import escom.libreria.info.cliente.jpa.DescuentoClientePK;
...
public class DescuentoClienteController implements Serializable{
    ...
    @EJB private escom.libreria.info.cliente.ejb.DescuentoClienteFacade ejbFacade;
    ...
    private void performDestroy() {
        try {
            getFacade().remove(current);
            JsfUtil.addSuccessMessage(ResourceBundle.getBundle("/Bundle").getString("DescuentoClienteDeleted"));
        } catch (Exception e) {
            JsfUtil.addErrorMessage(e, ResourceBundle.getBundle("/Bundle").getString("PersistenceErrorOccured"));
        }
    }
    ...
    public static class DescuentoClienteControllerConverter implements Converter {
        ...
        escom.libreria.info.cliente.jpa.DescuentoClientePK getKey(String value) {
            escom.libreria.info.cliente.jpa.DescuentoClientePK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new escom.libreria.info.cliente.jpa.DescuentoClientePK();
            key.setIdCliente(values[0]);
            key.setIdDescuento(Integer.parseInt(values[1]));
            return key;
        }

        String getStringKey(escom.libreria.info.cliente.jpa.DescuentoClientePK value) {
